...
## BENCH-FR-04 — 독립 데이터셋 출처 증명

상태: Done — 2026-08-18 공개 증거 manifest v5에서 검증.

공개 비교에 사용하는 데이터셋은 manifest의 독립성 자기선언만으로 승인하지 않는다. 검증자가 신뢰한 저자는 데이터셋 해시에 서명하고, 원어민 검수자는 자신에게 승인된 언어와 데이터셋 해시 및 PASS 판정에 서명해야 한다. 저자·언어별 검수자 목록은 manifest와 정확히 일치해야 하며 누락, 추가, 언어 간 서명 재사용을 거부한다.
