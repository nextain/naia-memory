...
6. 격리된 고정 표본에서 A/B와 B/A를 각각 3쌍씩 번갈아 실행한 6회 warm run
   중앙값과 전체 코퍼스
...
- 홀수 5회 교차 실행의 순서 불균형을 제거하기 위해 A/B와 B/A가 각각 3번인 6쌍으로
  고정하고, warm 중앙값과 캐시 없는 전체 코퍼스 속도 모두 1.5배를 넘어야 한다.
  두 실행의 peak RSS는 그대로 공개하며 warm/full-corpus 실패가 하나라도 있으면
  채택하지 않는다.
- `throughput-ab-analysis.ts`는 위 조건의 결정론적 계산기일 뿐 측정 출처를 증명하지
  않는다. 실제 채택 영수증은 실행 명령, A/B 정책 해시, 동일 호스트 정체성, 원시
  관측값 해시를 별도 실행 하네스로 결합해야 하며 그 전에는 속도 증거가 아니다.
- `throughput-ab-evidence.ts`는 실행 하네스가 낼 영수증의 입력 경계를 먼저 고정한다.
  스키마·벤치마크 ID, A/B 추론 모드와 서로 다른 정책 해시, 배치 크기와 입력 순서,
  Transformers 버전, 동일 부팅 ID, 절대 실행 파일·realpath 작업 디렉터리·명시적
  비밀 없는 환경·stdout·실제 `/proc/<pid>/cmdline`과 각각의 재계산 가능한 해시,
  canonical UTC 시각,
  벽시계 오차, 6쌍의 실제 AB/BA 실행 순서와 비중첩 chronology를 fail-closed로
  검증한 뒤에만 처리량 판정기를 호출한다. `throughput-ab-runner.ts`는 shell 없이
  부모 환경을 상속하지 않고 자식 프로세스를 직접 실행하며 PID/start tick/실제
  cmdline, 100ms 간격 process-tree
  aggregate VmRSS 표본, stdout, 종료 상태를 관측한다. 자식의 단일 JSON 영수증에
  label·정책 해시·실패 수·embedded/cached 문서 수가 모두 사전 기대값과 일치해야
  관측값을 반환한다. 이 RSS는 연속 최대값이 아니라 100ms 표본 최대값이며, 실행기
  자체도 외부 서명 원장은 아니므로 임의 JSON만으로 실행 진위를 주장하지 않는다.
- 장시간 전체 코퍼스 실행은 부팅 ID, 프로세스 시작 tick, 정확한 명령행, 감시 도구
  소스 해시를 실행 중 대조한다. 종료 뒤 결과 해시와 `/proc`의 누적 peak RSS가 같은
  감시 영수증에 없으면 최종 증거 생성기가 실패하도록 고정한다.
- 분석 영수증의 `passed`는 순위 비열등성만 뜻한다. 처리량과 정책 정체성은 별도
  증거가 없으면 채택 판정에 사용할 수 없다.

순위 판정기, 처리량 판정기, 처리량 증거 경계와 통제 실행기의 집중 테스트가
통과했다. 전체 회귀 테스트 수는 아래 검증 시점에 다시 기록하며 benchmark 포함
TypeScript typecheck도 통과해야 채택한다.
