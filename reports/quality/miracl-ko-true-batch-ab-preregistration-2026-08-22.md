...
- `throughput-ab-evidence.ts`는 실행 하네스가 낼 영수증의 입력 경계를 먼저 고정한다.
  ...
  Transformers 버전, 동일 부팅 ID, 실행 명령·stdout 해시, canonical UTC 시각,
  벽시계 오차, 6쌍의 실제 AB/BA 실행 순서와 비중첩 chronology를 fail-closed로
  검증한 뒤에만 처리량 판정기를 호출한다. 임의 JSON의 진위를 증명하는 도구는
  아니므로 통제된 자식 프로세스를 직접 실행하고 `/proc` 관측과 출력을 봉인하는
  실행기는 여전히 채택 전 필수다.
...
순위 판정기 전용 테스트 9개, 처리량 판정기 4개, 처리량 증거 경계 6개를 포함해
전체 테스트 113개 파일, 946개가 통과했다. benchmark 포함 TypeScript typecheck도
통과했다.
