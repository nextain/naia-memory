...
type PublicEvidenceReceipt = Omit<
	PublicEvidenceEngine,
	"executed" | "receiptPath" | "receiptSha256"
> & {
...
export function publicEvidenceSignaturePayload(
...
function evaluateManifest(
	...
	reject(
		manifest.schemaVersion !== "naia-memory-public-evidence-v3",
...
export function publicCaseOutputSha256(
	...
	return createHash("sha256")
		.update(canonicalJson({ engine, caseId, repetition, output }))
		.digest("hex");
}
...
function compareDataset(
	...
	if (!isDeepStrictEqual(languageCaseCounts, manifest.languageCaseCounts))
...
export async function evaluatePublicEvidenceFiles(
	...
	if (
		...
		decision.failures.push("manifest signature is untrusted or invalid");
	if (
		trustPolicy.approvedScoringPolicies[manifest.protocol.scoringPolicyId] !==
		manifest.protocol.scorerArtifactSha256
	)
		decision.failures.push("scoring policy is not approved by the verifier");
	let root: string;
	try {
		...
		return {
			promotable: false,
			failures: [...decision.failures, "evidence root is unreadable"],
		};
	}
	const evidence = [
		{
	...
	for (const item of evidence) {
		...
		try {
			...
			if (actual !== item.sha256)
				decision.failures.push(`${item.label} content hash mismatch`);
			if (item.label === "dataset") {
			...
			} else if (item.label === "scorer artifact") {
				try {
					const policy = JSON.parse(bytes.toString("utf8"));
					if (!isDeepStrictEqual(policy, PUBLIC_RETRIEVAL_SCORING_POLICY))
						decision.failures.push("scorer artifact semantics mismatch");
				} catch {
					decision.failures.push("scorer artifact JSON is invalid");
				}
			} else if ("engine" in item) {
				try {
					const receipt = JSON.parse(
						bytes.toString("utf8"),
					) as PublicEvidenceReceipt;
				...
				try {
					const review = JSON.parse(
						bytes.toString("utf8"),
					) as PublicAdversarialReview;
					decision.failures.push(
						...compareReview(review, manifest.adversarialReview),
					);
					if (
						!hasValidSignature(
						...
						decision.failures.push(
							"adversarial review signature is untrusted or invalid",
						);
				} catch {
