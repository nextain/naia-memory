...
export interface QueryIdentityLaunchReceipt {
	schemaVersion: "naia-memory-query-identity-launch-receipt-v1";
	oracleSha256: string;
...
export function createQueryIdentityLaunchArtifacts(input: {
	...
	const blindPacket = buildQueryIdentityBlindPacket(input.oracle);
...
export function scorePublicQueryIdentityRun(input: {
	...
	if (
		...
		input.launchReceipt.schemaVersion !==
			"naia-memory-query-identity-launch-receipt-v1" ||
