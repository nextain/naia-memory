	...
	score(query: string, docId: string): number {
		...
		for (const queryToken of queryTokens) {
			...
			total +=
				(idf * (tf * (this.k1 + 1))) /
				(tf +
					this.k1 * (1 - this.b + (this.b * docTokens.length) / this.avgDl));
		}
		return total;
	}
}
...
export function keywordScore(query: string, document: string): number {
	...
	for (const token of queryTokens)
		hits += documentTokens.has(token) ? 1 : lower.includes(token) ? 0.8 : 0;
	return hits / queryTokens.length;
}

/** Lexical overlap with document-length normalization for hybrid ranking. */
export function compactKeywordScore(query: string, document: string): number {
	const queryTokens = tokenize(query);
