...
export type StructuredQuery = Pick<StructuredFact, "subject" | "property">;
...
export function buildLLMQueryStructurer(
	...
	return async (query: string) => {
		const boundedQuery = query.trim().slice(0, options.maxQueryChars ?? DEFAULT_MAX_QUERY_CHARS);
		...
		try {
			...
			if (typeof value.subject !== "string" || typeof value.property !== "string") return undefined;
			const subject = value.subject.trim();
			const property = value.property.trim();
			return subject && property && subject.length <= 200 && property.length <= 200
				? { subject, property }
				: undefined;
		} catch {
			return undefined;
