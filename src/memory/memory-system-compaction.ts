import { buildDeterministicRecap, buildRecapFromRollingSummary, buildStructuredSections } from "./compaction-helpers.js";
...
export abstract class MemorySystemCompaction extends MemorySystemRolling {
	...
	}): Promise<{ summary: { role: "assistant"; content: string; timestamp?: number }; droppedCount: number; realtime?: boolean }> {
		const messages = input.messages;
		const rolling = input.sessionId ? this.rollingSummaries.get(input.sessionId) : undefined;
		...
		if (this.summarizer) {
			try {
				const polished = await this.summarizer({
					messages, keepTail: input.keepTail, targetTokens: input.targetTokens,
					...(input.sessionId !== undefined ? { sessionId: input.sessionId } : {}), seedSummary: recap,
				});
				...
				console.warn("[MemorySystem] compaction summarizer failed, using deterministic recap:", error);
			}
		}
		return { summary: { role: "assistant", content: finalContent, timestamp: Date.now() }, droppedCount: messages.length, realtime };
	}
	...
	async attachHandoff(blob: {
		...
		await this.encode({
			...
			role: "assistant", timestamp: blob.createdAt,
		}, context);
		for (const anchor of blob.anchors) {
			await this.encode({ content: `[Handoff anchor] ${anchor}`, role: "user", timestamp: blob.createdAt }, context);
		}
	}
}
